...
class Query {
    ...
    public static function getCar() {
        $db = DataBase::getInstance();
        $query = "select s.id as seller_id, s.name as seller_name, c.id as carId,mark.name as markName ,cat.name as catName,m.name as model,body.name as bName,
     c.modification,c.status,c.transmission,c.distance,dr.name as drivetrain,c.engine,c.price,c.fuel,c.viewcount,c.created_date,
            i.id as image_id,i.image from car c left join body on c.body_id=body.id left join category_car
             cat on c.category_car_id=cat.id left join model m on c.model_id=m.id left join mark on mark.id=m.mark_id left join image_car i
           on c.id=i.car_id left join seller as s on c.seller_id=s.id left join drivetrain as dr on dr.id =c.drivetrain_id order by c.created_date,s.id, c.id,i.id ;  ";
        $check = $db->query($query);
        if ($check) {
            if ($check->num_rows > 0) {
                $result = Error_message::Error_number(1000);
    ...
    public static function getCompany() {
        $db = DataBase::getInstance();
        $query = "select c.* ,ct.id type_id,ct.name type_name,img.id img_id,img.image_url  from company c left join image_company  img on c.id=img.company_id  join company_type ct on c.type_id=ct.id  order by c.order,ct.id asc;";
        $check = $db->query($query);
        if ($check) {
            if ($check->num_rows > 0) {
                $result = Error_message::Error_number(1000);
                ...
                while ($rows = mysqli_fetch_assoc($check)) {
                    ...
                    if ($comId != $rows['id']) {
                        ...
                        $imageIndex = -1;
                        $comId = $rows['id'];
                        $subrow[$carIndex]['id'] = $comId;
                        $subrow[$carIndex]['type_id'] = $rows['type_id'];
                        $subrow[$carIndex]['type_name'] = $rows['type_name'];
                        $subrow[$carIndex]['name'] = $rows['name'];
                    ...
                    if ($imageId != $rows['img_id']) {
                        $imageIndex++;
                        $imageId = $rows['img_id'];
                        $subrow[$carIndex]['image'][$imageIndex]['image_url'] = $rows['image_url'];
                    }
                }
                $result['data'] = $subrow;
            } else {
